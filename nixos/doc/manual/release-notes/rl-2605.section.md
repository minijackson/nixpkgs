...
- `services.prometheus` is now configured using the `settings` option,
  instead of the now deprecated `globalConfig`, `scrapeConfigs` options.
  This `settings` option will be converted into a YAML file as-is.
  This makes it possible to use any Prometheus parameter
  without having to use the `configText` option.

  To migrate, follow the evaluation warnings,
  and replace `globalConfig` by `settings.global`,
  `scrapeConfigs` by `settings.scrape_configs`, etc.

  To migrate your `rules` configuration,
  extract your rules to a separate file or use `pkgs.writeText`,
  then set `settings.rule_files = [ ./your/new/rules.yml ]`.

  Note that rules are now installed
  whether the Prometheus service is in agent mode or not.
